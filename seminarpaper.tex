...
For example there exists a simple meta model like shown in
\ref{fig:simple_metamodel}.

\begin{figure}
	\centering
	\includegraphics[angle=270,width=1\textwidth,natwidth=610,natheight=642]{figures/SimpleMM.pdf}
	\caption{Example of a simpel metamodel}
	\label{fig:simple_metamodel}
\end{figure}

For the next step we want create a binding reference (for example: b <- c1.a)
...
we have to save the attributes propertyName (`b`) and EType (`Type2`) of this
binding. Then we have to search from the original transformation a rule which
have as first OPE the `Type2` and next we have to save the type of OPE and this
is Class2. For the next step we need a reference from the input metamodel which
has type Class2 and save the name (`a`).
For the first implementation of the program it was not planned that we can use
the second order higher transformation as input for the original transformation.
That is the reason why we can search a rule which has not the type OPE `Type2`
in the second order higher transformation but only in the higher order
transformation. In this context there is no access to the class names of the
input and output model (`Type1`, `Type2`, `Class1`, `Class2`). For this missing
access there was a adaption of Main.java (Line 69-77) necassary. It was created
the possibility to add another input model - the original transformation
(AddBindingReference.atl).
...
Furthermore a adaption of TransfoRunnerEMFTVM.java and second order higher
transformation (line 4: IN_ATL:IN_ATL was added) was necassary.

In the following figure \ref{fig:addBindingReference} we see the original
implementation of the program and as part b in the figure we see the missing
reference which have to be added. 

\begin{figure}
	\centering
	\includegraphics[angle=270,width=1\textwidth,natwidth=610,natheight=642]{figures/AddBindingMissingReference.pdf}
